export function Card({ className = '', children }) {
  return (
    <div className={`bg-surface border border-hair rounded-2xl card-shadow ${className}`}>{children}</div>
  )
}
  ...
  return (
    <section className="mt-7">
      ...
      <div className="mb-3 ml-0.5 flex flex-wrap items-baseline gap-x-3 gap-y-1">
        <h2 className="text-[13px] font-semibold uppercase tracking-[0.08em] text-ink-sec">{title}</h2>
...
export function DayFilterBar({ label, onClear }) {
  return (
    <div className="flex flex-wrap items-center gap-2.5 rounded-xl border border-accent/45 bg-accent/10 px-3 py-2 text-[13px]">
      ...
      <button
        onClick={onClear}
        className="ml-auto cursor-pointer rounded-lg border border-hair bg-surface px-2.5 py-1 text-[12.5px] font-semibold text-ink-sec transition hover:border-axis hover:text-ink"
      >
        ✕ Torna ai 14 giorni
      </button>
    </div>
  ...
  return (
    <>
      <select
        ...
        className="min-w-0 rounded-xl border border-hair bg-surface px-2.5 py-2 text-[13px] font-semibold text-ink outline-none hover:border-axis sm:hidden"
      >
        {options.map((o) => (
      ...
      <div
        role="tablist"
        aria-label={ariaLabel}
        className="hidden gap-0.5 rounded-xl border border-hair bg-plane p-[3px] sm:inline-flex"
      >
        {options.map((o) => {
          ...
          return (
            <button
              ...
              className={`cursor-pointer rounded-lg px-3 py-1.5 text-[13px] font-semibold transition ${
                on ? 'bg-surface text-ink shadow-sm' : 'text-ink-sec hover:text-ink'
              }`}
            >
              {o.label}
            </button>
...
export function Message({ children, tone = 'info' }) {
  ...
  return <div className={`rounded-xl border bg-surface p-4 text-[13.5px] ${cls}`}>{children}</div>
}
...
export function IconButton({ title, onClick, children, active = false }) {
  return (
    <button
      ...
      aria-label={title}
      onClick={onClick}
      className={`inline-flex h-[38px] w-[38px] items-center justify-center rounded-xl border transition ${
        active ? 'border-accent text-accent' : 'border-hair text-ink hover:border-axis'
      } bg-surface`}
    >
      {children}
    </button>

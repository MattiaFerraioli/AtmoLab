export function Card({ className = '', children }) {
  return (
    <div className={`bg-surface border border-hair rounded-[20px] card-shadow ${className}`}>{children}</div>
  )
}
...
export function IconButton({ title, onClick, children, active = false }) {
  return (
    <button
      ...
      aria-label={title}
      onClick={onClick}
      className={`inline-flex h-[36px] w-[36px] items-center justify-center rounded-full transition ${
        active ? 'bg-accent/15 text-accent' : 'bg-fill text-ink hover:opacity-75'
      }`}
    >
      {children}
    </button>
